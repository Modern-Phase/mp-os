...
export const BUSINESS_RULES = {
  ALLOWED_UPGRADE_FROM_PLAN: PLANS.FREE,
  PAYMENT_METHODS: [
    "card",
...
export const UNAUTH_getDefaultPlan = internalQuery({
  handler: async (ctx): Promise<Doc<"plans"> | null> => {
    return ctx.db
      .query("plans")
...
export const createSubscriptionCheckout = action({
  ...
  handler: async (ctx, args): Promise<string | undefined> => {
    ...
    if (
      currentSubscription.plan.key !== BUSINESS_RULES.ALLOWED_UPGRADE_FROM_PLAN
    ) {
      return;
    }
    ...
    const checkout = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      payment_method_types: BUSINESS_RULES.PAYMENT_METHODS,
